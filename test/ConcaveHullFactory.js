...
function 正面(){
	...
	concaveSVG(flowerString, Infinity, 23);
}
...
function 側面(){
	...
	let draw = SVG('drawing').size(300, 300).viewbox(0,0,600,600);

	let flower = draw.svg(flowerString);
	let points = getPathPoints(flower, draw).filter(x=>typeof x[0] === 'number' && typeof x[1] === 'number');
	let polygon = concaveman(points, Infinity, 55);
	SVG.adopt(flower.select('#SvgjsG1684').members[0].node)
		.polygon()
		.plot(polygon)
		.fill('none')
		.stroke({ width: 3 })
